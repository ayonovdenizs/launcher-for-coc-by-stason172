...
Мод может подставить свою графику, не трогая сборку: в `config.launcher` есть
`assets.background` и `assets.icon` — пути относительно файла конфига.
Если файл не найден или поле пустое, используются встроенные картинки отсюда.
